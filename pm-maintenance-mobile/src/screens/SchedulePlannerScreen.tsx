    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTextWrap}>
          ...
          <Text style={styles.headerSubtitle}>Create PM tasks and generated due dates</Text>
        </View>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.inputRow}>
            ...
            <Pressable style={styles.dateButton} onPress={() => setDatePickerTarget("end")}>
              <Ionicons name="calendar-outline" size={18} color={colors.primary} />
              <View>
                <Text style={styles.dateLabel}>End date</Text>
                <Text style={styles.dateValue}>{endDate || "Optional"}</Text>
              </View>
            </Pressable>
          </View>
          {endDate ? (
            <Pressable style={styles.clearDateButton} onPress={() => setEndDate("")}>
              <Ionicons name="close-circle-outline" size={16} color="#626781" />
          ...
          {datePickerTarget && (
            <View style={styles.datePickerWrap}>
              <DateTimePicker
                value={parseIsoDate(datePickerTarget === "start" ? startDate : endDate)}
                mode="date"
          ...
          <TextInput style={styles.inputWide} placeholder="Occurrences" value={occurrences} onChangeText={setOccurrences} keyboardType="numeric" />
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.fieldHeaderRow}>
            <Text style={styles.fieldLabel}>Employees / operators</Text>
            {selectedAssignee ? <Text style={styles.fieldSelectedText}>{selectedAssignee.fullName}</Text> : null}
          </View>
          <View style={styles.employeeGrid}>
            {(context?.assignableEmployees ?? []).length ? (context?.assignableEmployees ?? []).map((employee) => (
              <EmployeeCard key={employee.employeeId} employee={employee} selected={employee.employeeId === assigneeId} onPress={() => setAssigneeId(employee.employeeId)} />
            )) : <Text style={styles.emptyInlineText}>No operators or technicians found for this line.</Text>}
          </View>
          <View style={styles.fieldHeaderRow}>
            <Text style={styles.fieldLabel}>Supervisors</Text>
            {selectedSupervisor ? <Text style={styles.fieldSelectedText}>{selectedSupervisor.fullName}</Text> : null}
          </View>
          <View style={styles.employeeGrid}>
            {(context?.supervisors ?? []).length ? (context?.supervisors ?? []).map((employee) => (
              <EmployeeCard key={employee.employeeId} employee={employee} selected={employee.employeeId === supervisorId} onPress={() => setSupervisorId(employee.employeeId)} />
            )) : <Text style={styles.emptyInlineText}>No supervisors found for this line.</Text>}
          </View>
        </View>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Planned schedules</Text>
          {tasks.length ? tasks.map((task) => (
            <Pressable key={`${task.taskScheduleId}-${task.stdTaskId}`} style={styles.taskCard} onPress={() => {
              ...
              <View style={styles.taskBody}>
                ...
                <Text style={styles.taskMeta} numberOfLines={2}>
                  {task.lineName} • {task.equipmentName} • {task.frequency}
                </Text>
          ...
          )) : <Text style={styles.emptyText}>No planned schedules yet.</Text>}
        </View>
      </ScrollView>
      ...
      <Modal visible={Boolean(selectedPlannedTask)} animationType="slide" transparent onRequestClose={() => {
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            <ScrollView contentContainerStyle={styles.modalList}>
              <Text style={styles.fieldLabel}>Due schedules</Text>
              {(selectedPlannedTask?.executions ?? []).map((execution) => {
                const selected = selectedExecution?.scheduleExecutionId === execution.scheduleExecutionId;
                return (
                  <Pressable
                    key={execution.scheduleExecutionId}
                    style={[styles.executionRow, selected && styles.executionRowSelected]}
                    onPress={() => setSelectedExecution(execution)}
                  >
                    <View style={styles.executionDateBox}>
                      <Text style={styles.executionDateText}>{execution.dueDate?.slice(0, 10)}</Text>
                      <Text style={styles.executionStatusText}>{execution.status}</Text>
                    </View>
                    <View style={styles.executionAssigneeWrap}>
                      <Text style={styles.executionAssigneeLabel}>Assigned to</Text>
                      <Text style={styles.executionAssigneeName} numberOfLines={1}>{execution.assigneeName || "Unassigned"}</Text>
                    </View>
                    <Ionicons name={selected ? "checkmark-circle" : "swap-horizontal-outline"} size={21} color={selected ? colors.primary : "#8B90A4"} />
                  </Pressable>
                );
              })}
              ...
              {selectedExecution ? (
                <>
                  <Text style={styles.fieldLabel}>Reassign selected due date</Text>
                  {(context?.assignableEmployees ?? []).map((employee) => (
                    <EmployeeCard
