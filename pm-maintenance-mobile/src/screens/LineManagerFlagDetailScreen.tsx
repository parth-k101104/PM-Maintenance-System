...
import { reviewFlagAsLineManager, scanFlagQr } from "../api/client";
...
type RoutePropType = RouteProp<RootStackParamList, "LineManagerFlagDetail">;

const STATUS_OPTIONS = [
  { label: "Potential Replacement", value: "POTENTIAL_REPLACEMENT" },
  { label: "Replacement Required", value: "REPLACEMENT_REQUIRED" },
  { label: "Replacement Initiated", value: "REPLACEMENT_INITIATED" },
  { label: "Close Flag", value: "CLOSED" },
];

const CRITICALITY_OPTIONS = ["LOW", "MEDIUM", "HIGH", "CRITICAL"];

export function LineManagerFlagDetailScreen() {
  ...
  const [criticality, setCriticality] = useState(flag.criticality);
  ...
  const submitReview = async () => {
    if (status === "CLOSED" && !closureReason.trim()) {
      Alert.alert("Required", "Please provide a closure reason.");
      return;
    }

    setSubmitting(true);
    try {
      await reviewFlagAsLineManager(authState.session!.token, flag.flagId, {
        newStatus: status,
        criticality: criticality,
        notes: notes,
        closureReason: status === "CLOSED" ? closureReason : undefined,
      });
      ...
      setMessageModal({
        ...
        type: "success",
        title: "Success",
        message: "Issue flag has been reviewed and updated.",
        goBackOnClose: true,
      });
    } catch (e: any) {
      setMessageModal({
        ...
        type: "failure",
        title: "Review failed",
        message: getBackendMessage(e, "Failed to review the flag."),
      });
    } finally {
      setSubmitting(false);
    }
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.detailCard}>
          ...
          <Text style={styles.label}>Current Criticality</Text>
        ...
        {loading ? (
          ...
          <View style={styles.workflowSection}>
            ...
            <View style={styles.notesSection}>
              <Text style={styles.sectionTitle}>Change Criticality</Text>
              <View style={styles.optionGrid}>
                {CRITICALITY_OPTIONS.map((option) => (
                  <Pressable
                    key={option}
                    style={[styles.optionChip, criticality === option && styles.optionChipSelected]}
                    onPress={() => setCriticality(option)}
                  >
                    <Text style={[styles.optionText, criticality === option && styles.optionTextSelected]}>
                      {option}
                    </Text>
                  </Pressable>
                ))}
            ...
            {status === "CLOSED" && (
              <View style={styles.notesSection}>
                <Text style={styles.sectionTitle}>Closure Reason</Text>
                <TextInput
                  style={styles.notesInput}
                  multiline
                  numberOfLines={2}
                  placeholder="Reason for closing the flag..."
                  value={closureReason}
            ...
            <View style={styles.notesSection}>
              ...
              <TextInput
                ...
                multiline
                numberOfLines={3}
                placeholder="Add any remarks for the operator or maintenance team..."
                value={notes}
